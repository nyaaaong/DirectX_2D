...
#pragma clang diagnostic ignored "-Wunknown-warning-option"         // warning: unknown warning group 'xxx'                      // not all warnings are known by all Clang versions and they tend to be rename-happy.. so ignoring warnings triggers DBG_NEW warnings on some configuration. Great!
#endif
...
static ImVec2           InputTextCalcTextSizeW(const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining = NULL, ImVec2* out_offset = NULL, bool stop_on_DBG_NEW_line = false);
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    // The old Combo() API exposed "popup_max_height_in_items". The DBG_NEW more general BeginCombo() API doesn't have/need it, but we emulate it here.
...
// - stb_sprintf.h supports several DBG_NEW modifiers which format numbers in a way that also makes them incompatible atof/atoi.
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (is_logarithmic)
    {
        ...
        float v_DBG_NEW_parametric = v_old_parametric + g.DragCurrentAccum;
        v_cur = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, v_DBG_NEW_parametric, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
    ...
    {
        // Convert to parametric space, apply delta, convert back
        float v_DBG_NEW_parametric = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, v_cur, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
        g.DragCurrentAccum -= (float)(v_DBG_NEW_parametric - v_old_ref_for_accum_remainder);
    }
    else
    {
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (t < 1.0)
            {
                FLOATTYPE v_DBG_NEW_off_f = (SIGNEDTYPE)(v_max - v_min) * t;
                result = (TYPE)((SIGNEDTYPE)v_min + (SIGNEDTYPE)(v_DBG_NEW_off_f + (FLOATTYPE)(v_min > v_max ? -0.5 : 0.5)));
            }
            else
            {
                result = v_max;
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Nav)
        {
            ...
            else if (g.SliderCurrentAccumDirty)
            {
                ...
                else
                {
                    ...
                    // Calculate what our "DBG_NEW" clicked_t will be, and thus how far we actually moved the slider, and subtract this from the accumulator
                    TYPE v_DBG_NEW = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, clicked_t, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                    if (!(flags & ImGuiSliderFlags_NoRoundToFormat))
                        v_DBG_NEW = RoundScalarWithFormatT<TYPE, SIGNEDTYPE>(format, data_type, v_DBG_NEW);
                    float DBG_NEW_clicked_t = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, v_DBG_NEW, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);

                    if (delta > 0)
                        g.SliderCurrentAccum -= ImMin(DBG_NEW_clicked_t - old_clicked_t, delta);
                    else
                        g.SliderCurrentAccum -= ImMax(DBG_NEW_clicked_t - old_clicked_t, delta);
                }
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min, const void* p_clamp_max)
{
    ...
    if (TempInputText(bb, id, label, data_buf, IM_ARRAYSIZE(data_buf), flags))
    {
        ...
        // Apply DBG_NEW value (or operations) then clamp
        ...
        // Only mark as edited if DBG_NEW value is different
...
static ImVec2 InputTextCalcTextSizeW(const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset, bool stop_on_DBG_NEW_line)
{
    ImGuiContext& g = *GImGui;
    ImFont* font = g.Font;
...
static bool STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj, int pos, const ImWchar* DBG_NEW_text, int DBG_NEW_text_len)
{
    const bool is_resizable = (obj->Flags & ImGuiInputTextFlags_CallbackResize) != 0;
    const int text_len = obj->CurLenW;
    IM_ASSERT(pos <= text_len);

    const int DBG_NEW_text_len_utf8 = ImTextCountUtf8BytesFromStr(DBG_NEW_text, DBG_NEW_text + DBG_NEW_text_len);
    if (!is_resizable && (DBG_NEW_text_len_utf8 + obj->CurLenA + 1 > obj->BufCapacityA))
        return false;

    // Grow internal buffer if needed
    if (DBG_NEW_text_len + text_len + 1 > obj->TextW.Size)
    {
        if (!is_resizable)
            return false;
        IM_ASSERT(text_len < obj->TextW.Size);
        obj->TextW.resize(text_len + ImClamp(DBG_NEW_text_len * 4, 32, ImMax(256, DBG_NEW_text_len)) + 1);
    }

    ImWchar* text = obj->TextW.Data;
    if (pos != text_len)
        memmove(text + pos + DBG_NEW_text_len, text + pos, (size_t)(text_len - pos) * sizeof(ImWchar));
    memcpy(text + pos, DBG_NEW_text, (size_t)DBG_NEW_text_len * sizeof(ImWchar));

    obj->Edited = true;
    obj->CurLenW += DBG_NEW_text_len;
    obj->CurLenA += DBG_NEW_text_len_utf8;
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* DBG_NEW_text, const char* DBG_NEW_text_end)
{
    const bool is_resizable = (Flags & ImGuiInputTextFlags_CallbackResize) != 0;
    const int DBG_NEW_text_len = DBG_NEW_text_end ? (int)(DBG_NEW_text_end - DBG_NEW_text) : (int)strlen(DBG_NEW_text);
    if (DBG_NEW_text_len + BufTextLen >= BufSize)
    {
        if (!is_resizable)
            return;
        ...
        int DBG_NEW_buf_size = BufTextLen + ImClamp(DBG_NEW_text_len * 4, 32, ImMax(256, DBG_NEW_text_len)) + 1;
        edit_state->TextA.reserve(DBG_NEW_buf_size + 1);
        Buf = edit_state->TextA.Data;
        BufSize = edit_state->BufCapacityA = DBG_NEW_buf_size;
    }

    if (BufTextLen != pos)
        memmove(Buf + pos + DBG_NEW_text_len, Buf + pos, (size_t)(BufTextLen - pos));
    memcpy(Buf + pos, DBG_NEW_text, (size_t)DBG_NEW_text_len * sizeof(char));
    Buf[BufTextLen + DBG_NEW_text_len] = '\0';

    if (CursorPos >= pos)
        CursorPos += DBG_NEW_text_len;
    SelectionStart = SelectionEnd = CursorPos;
    BufDirty = true;
    BufTextLen += DBG_NEW_text_len;
}
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    ...
    if (c < 0x20)
    {
        ...
        if (!pass)
            return false;
        apply_named_filters = false; // Override named filters below so DBG_NEWline and tabs can still be inserted.
    }
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        {
            bool ctrl_enter_for_DBG_NEW_line = (flags & ImGuiInputTextFlags_CtrlEnterForNewLine) != 0;
            if (!is_multiline || (ctrl_enter_for_DBG_NEW_line && !io.KeyCtrl) || (!ctrl_enter_for_DBG_NEW_line && io.KeyCtrl))
            {
    ...
    const char* apply_DBG_NEW_text = NULL;
    int apply_DBG_NEW_text_length = 0;
    if (g.ActiveId == id)
    {
        IM_ASSERT(state != NULL);
        if (cancel_edit)
        {
            ...
            if (!is_readonly && strcmp(buf, state->InitialTextA.Data) != 0)
            {
                ...
                apply_DBG_NEW_text = state->InitialTextA.Data;
                apply_DBG_NEW_text_length = state->InitialTextA.Size - 1;
                ImVector<ImWchar> w_text;
                if (apply_DBG_NEW_text_length > 0)
                {
                    w_text.resize(ImTextCountCharsFromUtf8(apply_DBG_NEW_text, apply_DBG_NEW_text + apply_DBG_NEW_text_length) + 1);
                    ImTextStrFromUtf8(w_text.Data, w_text.Size, apply_DBG_NEW_text, apply_DBG_NEW_text + apply_DBG_NEW_text_length);
                }
                stb_textedit_replace(state, &state->Stb, w_text.Data, (apply_DBG_NEW_text_length > 0) ? (w_text.Size - 1) : 0);
            }
        }
        ...
        {
            // Apply DBG_NEW value immediately - copy modified buffer back
            ...
            {
                apply_DBG_NEW_text = state->TextA.Data;
                apply_DBG_NEW_text_length = state->CurLenA;
            }
        }
    ...
    if (apply_DBG_NEW_text != NULL)
    {
        // We cannot test for 'backup_current_text_length != apply_DBG_NEW_text_length' here because we have no guarantee that the size
        // of our owned buffer matches the size of the string object held by the user, and by design we allow InputText() to be used
        // without any storage on user's side.
        IM_ASSERT(apply_DBG_NEW_text_length >= 0);
        if (is_resizable)
        {
            ImGuiInputTextCallbackData callback_data;
            ...
            callback_data.BufTextLen = apply_DBG_NEW_text_length;
            callback_data.BufSize = ImMax(buf_size, apply_DBG_NEW_text_length + 1);
            callback_data.UserData = callback_user_data;
            callback(&callback_data);
            buf = callback_data.Buf;
            buf_size = callback_data.BufSize;
            apply_DBG_NEW_text_length = ImMin(callback_data.BufTextLen, buf_size - 1);
            IM_ASSERT(apply_DBG_NEW_text_length <= buf_size);
        }
        //IMGUI_DEBUG_LOG("InputText(\"%s\"): apply_DBG_NEW_text length %d\n", label, apply_DBG_NEW_text_length);

        // If the underlying buffer resize was denied or not carried to the next frame, apply_DBG_NEW_text_length+1 may be >= buf_size.
        ImStrncpy(buf, apply_DBG_NEW_text, ImMin(apply_DBG_NEW_text_length + 1, buf_size));
        value_changed = true;
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    {
        float DBG_NEW_H, DBG_NEW_S, DBG_NEW_V;
        ColorConvertRGBtoHSV(col[0], col[1], col[2], DBG_NEW_H, DBG_NEW_S, DBG_NEW_V);
        if (DBG_NEW_H <= 0 && H > 0)
        {
            if (DBG_NEW_V <= 0 && V != DBG_NEW_V)
                ColorConvertHSVtoRGB(H, S, DBG_NEW_V <= 0 ? V * 0.5f : DBG_NEW_V, col[0], col[1], col[2]);
            else if (DBG_NEW_S <= 0)
                ColorConvertHSVtoRGB(H, DBG_NEW_S <= 0 ? S * 0.5f : DBG_NEW_S, DBG_NEW_V, col[0], col[1], col[2]);
        }
    }
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags, ImGuiDockNode* dock_node)
{
    ...
    // Ensure correct ordering when toggling ImGuiTabBarFlags_Reorderable flag, or when a DBG_NEW tab was added while being not reorderable
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        // Additionally, when using TabBarAddTab() to manipulate tab bar order we occasionally insert DBG_NEW tabs that don't have a width yet,
...
void ImGui::TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow* window)
{
    ...
    ImGuiTabItem DBG_NEW_tab;
    DBG_NEW_tab.ID = window->ID;
    DBG_NEW_tab.Flags = tab_flags;
    DBG_NEW_tab.LastFrameVisible = tab_bar->CurrFrameVisible;   // Required so BeginTabBar() doesn't ditch the tab
    if (DBG_NEW_tab.LastFrameVisible == -1)
        DBG_NEW_tab.LastFrameVisible = g.FrameCount - 1;
    DBG_NEW_tab.Window = window;                                // Required so tab bar layout can compute the tab width before tab submission
    tab_bar->Tabs.push_back(DBG_NEW_tab);
}
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // Note that tab_is_DBG_NEW is not necessarily the same as tab_appearing! When a tab bar stops being submitted
    // and then gets submitted again, the tabs will have 'tab_appearing=true' but 'tab_is_DBG_NEW=false'.
    if (tab_appearing && (!tab_bar_appearing || tab_is_DBG_NEW))
    {
        ItemAdd(ImRect(), id, NULL, ImGuiItemFlags_NoNav | ImGuiItemFlags_NoNavDefaultFocus);
        if (is_tab_button)
            return false;
