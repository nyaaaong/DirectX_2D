...
// - TableNextRow()                             user begin into a DBG_NEW row (also automatically called by TableHeadersRow())
//    | TableEndRow()                           - finish existing row
//    | TableBeginRow()                         - add a DBG_NEW row
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    const float DBG_NEW_ref_scale_unit = g.FontSize; // g.Font->GetCharAdvance('A') ?
    if (table->RefScale != 0.0f && table->RefScale != DBG_NEW_ref_scale_unit)
    {
        const float scale_factor = DBG_NEW_ref_scale_unit / table->RefScale;
        //IMGUI_DEBUG_LOG("[table] %08X RefScaleUnit %.3f -> %.3f, scaling width by %.3f\n", table->ID, table->RefScaleUnit, DBG_NEW_ref_scale_unit, scale_factor);
        for (int n = 0; n < columns_count; n++)
            table->Columns[n].WidthRequest = table->Columns[n].WidthRequest * scale_factor;
    }
    table->RefScale = DBG_NEW_ref_scale_unit;
...
void    ImGui::EndTable()
{
    ...
    if (table->ResizedColumn != -1 && table->InstanceCurrent == table->InstanceInteracted)
    {
        ...
        const float DBG_NEW_x2 = (g.IO.MousePos.x - g.ActiveIdClickOffset.x + TABLE_RESIZE_SEPARATOR_HALF_THICKNESS);
        const float DBG_NEW_width = ImFloor(DBG_NEW_x2 - column->MinX - table->CellSpacingX1 - table->CellPaddingX * 2.0f);
        table->ResizedColumnNextWidth = DBG_NEW_width;
    }
...
// [Public] Starts into the first cell of a DBG_NEW row
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    // get the DBG_NEW cursor position.
    ...
    if (unfreeze_rows_actual)
    {
        ...
        // Update cliprect ahead of TableBeginCell() so clipper can access to DBG_NEW ClipRect->Min.y
...
// [Public] Append into the next column, wrap and create a DBG_NEW row when already on last column
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    // (old_a + old_b == DBG_NEW_a + DBG_NEW_b) --> (DBG_NEW_a == old_a + old_b - DBG_NEW_b)
    ...
    if (merge_group_mask != 0)
    {
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                {
                    // Copy + overwrite DBG_NEW clip rect
...
// [Main] 3: TableSaveSettings()               When table properties are modified, serialize Table data into bound or DBG_NEW TableSettings, mark .ini as dirty.
...
void ImGui::TableGcCompactSettings()
{
    ...
    if (required_memory == g.SettingsTables.Buf.Size)
        return;
    ImChunkStream<ImGuiTableSettings> DBG_NEW_chunk_stream;
    DBG_NEW_chunk_stream.Buf.reserve(required_memory);
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
        if (settings->ID != 0)
            memcpy(DBG_NEW_chunk_stream.alloc_chunk(TableSettingsCalcChunkSize(settings->ColumnsCount)), settings, TableSettingsCalcChunkSize(settings->ColumnsCount));
    g.SettingsTables.swap(DBG_NEW_chunk_stream);
}
