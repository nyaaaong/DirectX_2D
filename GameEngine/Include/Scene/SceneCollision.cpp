...
{
	if (!m_Section)
		m_Section = DBG_NEW CollisionSectionInfo;
...
void CSceneCollision::CreateSection()
{
	...
	for (int z = 0; z < m_Section->CountZ; ++z)
	{
		for (int y = 0; y < m_Section->CountY; ++y)
		{
			...
			{
				CCollisionSection* Section = DBG_NEW CCollisionSection;

				Section->Init(x, y, z, z * (m_Section->CountX * m_Section->CountY) + y * m_Section->CountX + x, m_Section->Min, m_Section->Max, m_Section->SectionSize,
					m_Section->SectionTotalSize);
