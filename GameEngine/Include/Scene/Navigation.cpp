...
bool CNavigation::FindPath(const Vector3& Start, const Vector3& End, std::list<Vector3>& vecPath)
{
	...
	if (EndNode->TileType == Tile_Type::T_Wall)
		return false;
...
NavNode* CNavigation::GetRectNodeTop(NavNode* Node, NavNode* EndNode, const Vector3& End, bool Digonal)
{
	...
	while (true)
	{
		...
		else if (CheckNode->TileType == Tile_Type::T_Wall)
			return nullptr;
		...
		{
			if (m_vecNode[CornerY * m_CountX + CornerX]->TileType == Tile_Type::T_Wall &&
				m_vecNode[(CornerY + 1) * m_CountX + CornerX]->TileType == Tile_Type::T_Normal)
			{
				return CheckNode;
			}
		}
...
NavNode* CNavigation::GetRectNodeRightTop(NavNode* Node, NavNode* EndNode, const Vector3& End)
{
	...
	while (true)
	{
		...
		{
			if (m_vecNode[CornerY * m_CountX + CornerX]->TileType == Tile_Type::T_Wall &&
				m_vecNode[CornerY * m_CountX + (CornerX + 1)]->TileType == Tile_Type::T_Normal)
			{
				return CheckNode;
			}
		}
