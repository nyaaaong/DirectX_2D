...
struct IMGUI_API ImGuiDockNode
{
    ...
    bool                    WantMouseMove           :1; // After a node extraction we need to transition toward moving the DBG_NEWly created host window
...
struct ImGuiSettingsHandler
{
    ...
    void*       (*ReadOpenFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler, const char* name);              // Read: Called when entering into a DBG_NEW ini entry e.g. "[Window][Name]"
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2                  CurrLineSize;
    ImVec2                  PrevLineSize;
    float                   CurrLineTextBaseOffset; // Baseline offset (0.0f by default on a DBG_NEW line, generally == style.FramePadding.y when a framed item has been added).
...
struct IMGUI_API ImGuiTabBar
{
    ...
    bool                TabsAddedNew;           // Set to true when a DBG_NEW tab item or button has been added to the tab bar during last frame
...
namespace ImGui
{
    ...
    // Viewports
    IMGUI_API void          TranslateWindowsInViewport(ImGuiViewportP* viewport, const ImVec2& old_pos, const ImVec2& DBG_NEW_pos);
...
//#endif
    ...
    IMGUI_API void          PushOverrideID(ImGuiID id);     // Push given value as-is at the top of the ID stack (whereas PushID combines old and DBG_NEW hashes)
